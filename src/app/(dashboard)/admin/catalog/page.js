import {
  ...
  getAdminCatalogKits,
  getAdminMerchandiseCategories,
  getAdminMerchandiseItems,
} from '@/actions/catalog';
import Category from '@/components/admin/catalog/category/Category';
...
function resolveTab(rawTab) {
  // Merchandise naming hidden; product CRUD lives under "produk".
  if (rawTab === 'merchandise' || rawTab === 'produk') return 'produk';
  if (rawTab === 'category') return 'category';
...
export default async function AdminCatalogPage({ searchParams }) {
  ...
  const merchandiseItems = activeTab === 'produk' ? await getAdminMerchandiseItems() : [];
  const categories = activeTab === 'produk' || activeTab === 'category' ? await getAdminMerchandiseCategories() : [];
    ...
    <section className="space-y-5">
      <div className="rounded-xl border border-slate-200 bg-gradient-to-br from-blue-50 via-cyan-50 to-slate-50 p-4 sm:p-5">
        ...
        <h2 className="mt-1 font-display text-2xl font-semibold text-slate-900">
          Kelola Kit, Kategori, Produk, dan File Katalog
        </h2>
        <p className="mt-2 text-sm text-slate-600">Gunakan menu di bawah untuk mengelola data katalog website.</p>
      </div>
      ...
      <div className="rounded-xl border border-slate-200 bg-white p-2">
        <div className="flex flex-wrap items-center gap-2">
          <div className="flex flex-wrap gap-2">
            ...
            <a
              href="/admin/catalog?tab=category"
              className={`inline-flex items-center rounded-lg px-4 py-2 text-sm font-semibold transition ${
                activeTab === 'category'
                  ? 'bg-blue-600 text-white shadow-sm'
                  : 'bg-slate-100 text-slate-700 hover:bg-slate-200'
